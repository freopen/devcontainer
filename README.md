...
named volume called `codex-in-devcontainer`.
...
Replace `vscode` with a user provided by the container image. The configuration
must explicitly set either `remoteUser` or `containerUser`, and
`updateRemoteUserUID` must remain `true` (its default). Every container sharing
the volume must resolve that user to the same numeric UID; otherwise ownership
and write access are not guaranteed. These requirements apply whether the
Feature is configured per project or through `dev.containers.defaultFeatures`.
...
2. mounts the named volume `codex-in-devcontainer` at `/codex`;
3. replaces the remote user's `~/.codex` with a link to `/codex`.

The named volume can be inspected with:

```sh
docker volume inspect codex-in-devcontainer
```

Because this storage can include credentials and conversation history, only
use the Feature in dev containers that you trust. Reserve the
`codex-in-devcontainer` volume name for this Feature; do not use an existing
volume with that name for unrelated data.
...
uses the shared `codex-in-devcontainer` volume without exposing the host's
`~/.codex`.
